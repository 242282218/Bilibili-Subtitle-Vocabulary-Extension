...
const SETTINGS_STORAGE_KEYS = sharedSettings && Array.isArray(sharedSettings.SETTINGS_STORAGE_KEYS)
...
function getLearningStatusLabel(status) {
  ...
  if (normalized === "seen" || normalized === "learning" || normalized === "reviewing") return "已遇见";
...
function getHeroMetricMeta(type, value) {
  ...
  if (type === "maxReplace") {
    const count = Math.min(5, Math.max(1, Math.floor(Number(value) || DEFAULT_SETTINGS.maxReplaceCount)));
    if (count >= 4) {
      return "高密度命中";
    }
    if (count <= 1) {
...
function updateMockPreview(settings) {
  ...
  const ratio = Math.min(0.3, Math.max(0.1, Number(snapshot.replaceRatio) || DEFAULT_SETTINGS.replaceRatio));
  const maxReplaceCount = Math.min(5, Math.max(1, Math.floor(Number(snapshot.maxReplaceCount) || DEFAULT_SETTINGS.maxReplaceCount)));
...
function readLearningDashboard(callback) {
  ...
  chrome.storage.local.get([
    ...
    const queue = learningState.normalizeReviewQueue(payload ? payload[REVIEW_QUEUE_STORAGE_KEY] : null);
    if (Object.keys(stats).length === 0 && payload && payload[WORD_STATS_STORAGE_KEY] && typeof payload[WORD_STATS_STORAGE_KEY] === "object") {
    ...
    const summary = payload && payload[LEARNING_SUMMARY_STORAGE_KEY] && typeof payload[LEARNING_SUMMARY_STORAGE_KEY] === "object"
...
function handleQuickReviewFeedback(feedback) {
  ...
  const normalizedAction = String(feedback || "").trim().toLowerCase();
  const now = Date.now();
...
function applySettingsToUI(settings) {
  ...
  if (maxReplaceCountInput) {
    maxReplaceCountInput.value = String(runtimeSettings.maxReplaceCount || DEFAULT_SETTINGS.maxReplaceCount);
  }
...
function buildSettingsPreviewFallback(settings) {
  ...
  const ratio = Math.min(0.3, Math.max(0.1, Number(settings.replaceRatio) || DEFAULT_SETTINGS.replaceRatio));
  const maxReplaceCount = Math.min(5, Math.max(1, Math.floor(Number(settings.maxReplaceCount) || DEFAULT_SETTINGS.maxReplaceCount)));
  const targetCefr = normalizeTargetCefr(settings.targetCefr);
  const activeLevels = Array.isArray(settings.activeLevels) && settings.activeLevels.length
...
function toggleReviewDanmaku() {
  ...
  setButtonBusy(reviewDanmakuButton, nextValue ? "启动中..." : "停止中...", getReviewDanmakuButtonLabel(reviewDanmakuEnabled));
...
function bindEvents() {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (changes[LEARNING_WORD_STATS_STORAGE_KEY] || changes[REVIEW_QUEUE_STORAGE_KEY] || changes[LEARNING_SUMMARY_STORAGE_KEY]) {
