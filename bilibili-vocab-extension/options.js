...
async function loadLearningStats() {
  try {
    ...
    const savedCount = getWordStatRecords(wordStats).filter(
      (word) => word.status === 'saved'
    ).length;
...
async function exportVocabularyBook(format = 'json') {
  try {
    ...
    link.setAttribute(
      'download',
      `bilibili-vocab-book-${new Date().toISOString().slice(0, 10)}.${exportPayload.extension}`
    );
  ...
  input.onchange = async (e) => {
    try {
      ...
      const normalized = sharedSettings
        ? sharedSettings.normalizeSettings(importedSettings)
        : normalizeSettings(importedSettings);
...
function renderRecommendationList(input) {
  if (Array.isArray(input)) {
    input.forEach(({ node, text, tone }) => {
      ...
      node.dataset.tone =
        getRecommendationColor(tone) === 'default' ? 'neutral' : getRecommendationColor(tone);
    });
    return;
  }
...
function initScenePicker() {
  sceneCards.forEach((card) => {
    const applyPreset = () => {
      ...
      setStatus(
        `已应用${card.querySelector('.hub-scenario-card__title')?.textContent || '策略'}预设`,
        1400
      );
    };
