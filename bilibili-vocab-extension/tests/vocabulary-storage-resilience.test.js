...
test("loadVocabulary: should keep level data when storage read fails", async (t) => {
  ...
  assert.equal(entries.some((entry) => entry.word === "cet4-word"), true);
  assert.equal(entries.some((entry) => entry.word === "toefl-word"), true);
});
...
test("refreshLearningStateFromStorage: should reject on storage read failure", async (t) => {
  ...
  await assert.rejects(
    vocabulary.refreshLearningStateFromStorage(),
    /mock read failed/
  );
});
      ...
      local: {
        get(_keys, callback) {
          ...
          callback({
            ...
            [learningState.STORAGE_KEYS.LEARNING_SUMMARY]: learningState.buildLearningSummary(stats, queue)
          });
        },
        set(_payload, callback) {
