    ...
    storage: {
      ...
      local: {
        get(keysOrDefaults, callback) {
          const failed =
            typeof shouldFailGet === 'function' ? shouldFailGet(keysOrDefaults) : false;
  ...
  withPopupRuntime(
    {
      ...
      shouldFailGet(keysOrDefaults) {
        return (
          Array.isArray(keysOrDefaults) && keysOrDefaults.includes(LEARNING_WORD_STATS_STORAGE_KEY)
        );
      },
    },
    (documentStub) => {
  ...
  withPopupRuntime(
    {
      ...
      shouldFailGet(keysOrDefaults) {
        return (
          Array.isArray(keysOrDefaults) &&
          keysOrDefaults.length === 1 &&
          keysOrDefaults[0] === WORD_STATS_STORAGE_KEY
        );
      },
    },
    (documentStub) => {
