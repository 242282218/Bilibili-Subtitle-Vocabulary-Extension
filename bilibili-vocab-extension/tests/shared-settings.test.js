...
test("shared settings: should expose defaults and scene presets", () => {
  assert.equal(loadError, null, loadError ? loadError.message : "sharedSettings.js should be loadable");

  assert.deepEqual(sharedSettings.DEFAULT_SETTINGS.activeLevels, ["CET4", "CET6", "KAOYAN", "IELTS", "TOEFL"]);
...
test("shared settings: should derive preview copy and preset key from normalized settings", () => {
  ...
  assert.equal(sharedSettings.getHeroMetricMeta("reviewSpeed", intensive.reviewDanmakuSpeed), "冲刺高频");
  assert.match(
    sharedSettings.buildSettingsPreview(intensive),
    /30%/
  );
  assert.match(
    sharedSettings.buildSettingsPreview(intensive),
...
test("shared settings v3: should resolve effective runtime by active profile and global controls", () => {
  ...
  const runtimeBlocked = sharedSettings.resolveEffectiveRuntime(settingsV3, { hostname: "example.com" });
  const runtimeAllowed = sharedSettings.resolveEffectiveRuntime(settingsV3, { hostname: "video.test" });
