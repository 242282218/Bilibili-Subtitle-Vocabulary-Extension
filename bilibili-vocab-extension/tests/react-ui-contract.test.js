...
test('react ui contract: manifest should not eagerly inject dist overlay bundle', () => {
  const manifest = readManifest();
  const firstContentScript = Array.isArray(manifest.content_scripts)
    ? manifest.content_scripts[0]
    : null;
  const scriptList =
    firstContentScript && Array.isArray(firstContentScript.js) ? firstContentScript.js : [];
...
test('react ui contract: content stylesheet should stay self-contained', () => {
  ...
  assert.match(
    stylesheet,
    /:is\(\.options-body,\s*\.popup-body\)\s*\{[\s\S]*--font-sans:\s*"Avenir Next"/
  );
  assert.match(
    stylesheet,
    /:is\(\.options-body,\s*\.popup-body\)\s*\{[\s\S]*--font-display:\s*"Iowan Old Style"/
  );
...
test('react ui contract: shared content styles should not depend on redesign tokens', () => {
  const stylesheet = readProjectFile('styles.css');
  const [contentSection] = stylesheet.split('/* ===== §1 Design Tokens');

  assert.match(contentSection, /\.subtitle-container\s*\{[\s\S]*color:\s*#ffffff;/);
  assert.match(contentSection, /\.subtitle-container\s*\{[\s\S]*padding:\s*4px 8px;/);
  assert.match(contentSection, /\.bili-vocab-word\s*\{[\s\S]*transition:\s*[\s\S]*180ms ease/);
  assert.match(contentSection, /\.level-cet4\s*\{\s*color:\s*#4a90e2;/);
  assert.doesNotMatch(
    contentSection,
    /var\(--(?:space-1|space-2|radius-xs|text-primary|line-height-normal|shadow-md|duration-normal|ease-out|focus-border|level-cet4|level-cet6|level-kaoyan|level-ielts|level-toefl)\)/
  );
});

test('react ui contract: legacy dashboard globals should stay scoped to extension pages', () => {
  const stylesheet = readProjectFile('styles.css');

  assert.doesNotMatch(stylesheet, /(^|\n):root\s*\{/m);
  assert.doesNotMatch(
    stylesheet,
    /(^|\n)\.(?:action-row|primary-button|secondary-button|danger-button|toast-message|status-line|recommendation-list)\b/m
  );
  assert.match(stylesheet, /:is\(\.options-body,\s*\.popup-body\)\s+\.primary-button\s*\{/);
  assert.match(stylesheet, /:is\(\.options-body,\s*\.popup-body\)\s+\.action-row\s*\{/);
  assert.match(stylesheet, /:is\(\.options-body,\s*\.popup-body\)\s+\.toast-message\s*\{/);
});
...
test('react ui contract: content script should lazy-load dist overlay bundle', () => {
  ...
  assert.match(contentScript, /import\(chrome\.runtime\.getURL\((['"])dist\/overlay\.js\1\)\)/);
