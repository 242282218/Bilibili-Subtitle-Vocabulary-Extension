...
test('check overlay size: should write report and summary when bundle is within budget', () => {
  ...
  try {
    ...
    assert.equal(report.file, 'bundle/overlay.js');
    ...
    assert.equal(parsedReport.file, 'bundle/overlay.js');
...
test('check overlay size: normalizeCheckedFilePath should keep project-relative path when possible', () => {
  const workspace = createTempDir();
  try {
    const overlayFile = path.join(workspace, 'dist', 'overlay.js');
    const normalized = normalizeCheckedFilePath(overlayFile, workspace);

    assert.equal(normalized, 'dist/overlay.js');
  } finally {
    fs.rmSync(workspace, { recursive: true, force: true });
  }
});
