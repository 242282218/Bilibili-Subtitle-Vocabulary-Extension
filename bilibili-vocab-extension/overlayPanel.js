(function (globalScope) {
  ...
  const uiStateMachine = globalScope.SettingsUiStateMachine || (typeof require === "function" ? require("./settingsUiStateMachine.js") : null);
  ...
  const RESIZE_STORAGE_KEYS = ["overlayPanelWidth", "overlayPanelHeight", "overlayPanelCollapsed", "overlayPanelHidden", "overlayPanelOffsetRight", "overlayPanelOffsetBottom"];
  const OVERLAY_INSTANCE_KEY = "__BILI_VOCAB_OVERLAY_INSTANCE__";

  if (sharedSettings) {
    Object.assign(DEFAULT_SETTINGS, sharedSettings.DEFAULT_SETTINGS);
  }
  const LEARNING_STATE_KEYS = learningState ? learningState.STORAGE_KEYS : {
  ...
  function clampOverlayOffsetRight(value) {
    ...
    return Math.min(OVERLAY_MAX_OFFSET_RIGHT, Math.max(OVERLAY_MIN_OFFSET_RIGHT, Math.round(offset)));
  }
  ...
  function clampOverlayOffsetBottom(value) {
    ...
    return Math.min(OVERLAY_MAX_OFFSET_BOTTOM, Math.max(OVERLAY_MIN_OFFSET_BOTTOM, Math.round(offset)));
  }
  ...
  function getMockSubtitlePreview(settings) {
    ...
    const count = Math.min(words.length, Math.max(1, Math.min(normalized.maxReplaceCount, density)));
  ...
  function normalizeOverlaySettings(settings) {
    ...
    return {
      ...
      reviewDanmakuSpeed: normalizedShared ? normalizedShared.reviewDanmakuSpeed : normalizeReviewDanmakuSpeed(source.reviewDanmakuSpeed),
      vocabularyMode: normalizedShared ? normalizedShared.vocabularyMode : DEFAULT_SETTINGS.vocabularyMode,
      examPreference: normalizedShared ? normalizedShared.examPreference : DEFAULT_SETTINGS.examPreference,
      ...
      replaceRatio: normalizedShared
        ...
        : Math.min(0.3, Math.max(0.1, Number(source.replaceRatio) || DEFAULT_SETTINGS.replaceRatio)),
      maxReplaceCount: normalizedShared
        ? normalizedShared.maxReplaceCount
        : Math.min(5, Math.max(1, Math.floor(Number(source.maxReplaceCount) || DEFAULT_SETTINGS.maxReplaceCount))),
      targetCefr: normalizedShared ? normalizedShared.targetCefr : normalizeTargetCefr(source.targetCefr),
  ...
  function mountOverlayPanel() {
    ...
    function collectLevels() {
      const checked = getLevelInputs().filter((input) => input.checked).map((input) => input.value);
    ...
    function getLearningSummaryData() {
      if (!globalScope.VocabularyModule || typeof globalScope.VocabularyModule.getLearningSummary !== "function") {
        return {
          todayCount: 0,
          newCount: 0,
          masteredCount: 0
    ...
    function getReviewQueueData() {
      if (!globalScope.VocabularyModule || typeof globalScope.VocabularyModule.getReviewQueue !== "function") {
        return [];
      }
    ...
    function updateAutoSaveState(savedOrState) {
      ...
      if (uiStateMachine && typeof uiStateMachine.getStateMessage === "function") {
        const state = typeof savedOrState === "string" ? savedOrState : (savedOrState ? "synced" : "dirty");
        autoSaveStateNode.textContent = uiStateMachine.getStateMessage(state, { channel: "autosave" });
        return;
      }
    ...
    function applyPanelState(settings) {
      ...
      panel.style.height = current.overlayPanelCollapsed ? "auto" : `${current.overlayPanelHeight}px`;
    ...
    [enabledInput, maxReplaceInput, cefrInput, reviewSpeedInput, vocabularyModeInput, examPreferenceInput].forEach((input) => {
