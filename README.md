...
- 清空“已收藏”后会立即同步学习概览；Popup / Overlay 的最近词汇状态不会继续残留旧的“已收藏”标签
...
- Bilibili 视频页中的 shipped React Overlay 现已支持 `上一句 / 重播本句 / 下一句` 字幕导航，并展示当前字幕在整条时间轴中的位置
- 句级导航复用 Bilibili 官方字幕轨道；如果当前视频暂时没有可用字幕轨，会在面板内直接提示而不是静默失效
- 字幕时间轴缓存现在按 Bilibili 视频标识自动失效；站内切视频/切分 P 时不会沿用上一条视频的旧时间轴，无字幕视频也不会在打开 Popup 后持续重复拉播放器字幕 API
- Overlay 的字幕导航状态现在直接复用 content-script 的统一运行时桥；进度、句段切换和切视频后的 loading 态会跟随播放器事件即时刷新，不再靠 `450ms` 轮询 `location.href` 与重复重读时间轴
- shipped React Popup 现在也会读取当前标签页的字幕导航快照，可直接执行 `上一句 / 重播本句 / 下一句`，不必先切回视频页再操作
- Popup 在保持打开期间会通过活动标签页订阅通道实时同步字幕导航状态；切句、拖动进度或切换活动标签页后，进度卡会立即跟上
- 如果当前标签页不是受支持的视频页，Popup 会明确提示“当前标签页暂无字幕导航”，而不是显示一个不可用的假按钮组
